...
function buildPrompt(profile, recentSessions, loads) {
  const allowed = EQUIPMENT_MAP[profile.equipmentAccess] ?? EQUIPMENT_MAP.full_gym;
...
- Time available per session: ${profile.sessionMinutes ?? 60} minutes
...
3. Fit each day inside ${profile.sessionMinutes ?? 60} minutes — roughly 3 minutes per working set including rest, so cap total sets accordingly.
