...
export function Button({
  ...
  label: string;
  onPress: () => void;
  variant?: 'primary' | 'secondary' | 'ghost' | 'danger' | 'success' | 'bronze';
  size?: 'sm' | 'md' | 'lg';
  disabled?: boolean;
  loading?: boolean;
  style?: ViewStyle;
  ...
  const fg =
    variant === 'primary' || variant === 'success' || variant === 'bronze'
      ? colors.onAccent
      : variant === 'danger'
      ? colors.danger
      : variant === 'ghost'
  ...
  const handlePress = () => {
    (variant === 'danger' ? haptics.tapMedium : haptics.tap)();
    onPress();
  };
...
export function Chip({
  ...
  const handlePress = onPress
    ...
    : undefined;
  return handlePress ? <Pressable onPress={handlePress}>{content}</Pressable> : content;
}
