/**
 * ATLAS — the three palettes.
 *
 * A theme swaps tokens and nothing else. The colour language is fixed across
 * all three and must never be re-taught: the accent always means fatigued,
 * neglected, or the primary action, and the bronze token always means
 * recovering, complete, a record, or Premium (a colour value every palette
 * carries — nothing to do with the removed "Bronze" theme). Someone who
 * learns the app in Obsidian and switches to Marble should not have to
 * re-read a single chart.
 *
...
export type ThemeName = 'obsidian' | 'marble' | 'stone';
...
const marble: Palette = {
  ...
  // has to be pulled meaningfully darker than on the dark themes or Fresh/Ready
  // both vanish into the page — caught after Fresh/Ready read as invisible
...
/** Quarried limestone. Cool, grey, the quietest of the three. */
const stone: Palette = {
  name: 'stone',
  label: 'Stone',
...
export const THEME_ORDER: ThemeName[] = ['obsidian', 'marble', 'stone'];
